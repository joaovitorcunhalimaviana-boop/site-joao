...
export default function MayoCalculator({
  ...
  const getScoreInterpretation = (score: number) => {
    if (score === 0) return { text: 'Mucosa normal', color: 'green' as const }
    if (score <= 2) return { text: 'Doença leve', color: 'yellow' as const }
    if (score <= 6) return { text: 'Doença moderada', color: 'yellow' as const }
    return { text: 'Doença severa', color: 'red' as const }
  }
