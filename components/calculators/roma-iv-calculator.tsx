...
import BaseCalculator, { CalculatorCard, CalculatorQuestion, CalculatorResult } from './base-calculator'
...
const romaCriteria = [
  {
    id: 'recurrent_pain',
    question: 'Dor abdominal recorrente, em média, pelo menos 1 dia por semana nos últimos 3 meses',
    required: true
  },
  {
    id: 'pain_defecation',
...
const siiSubtypes = {
  'SII-C': {
    ...
    description: 'Fezes duras ou em caroços ≥25% e fezes moles ≤25% das evacuações'
  },
  'SII-D': {
    name: 'SII com Diarreia',
    description: 'Fezes moles ≥25% e fezes duras ou em caroços ≤25% das evacuações'
  },
  'SII-M': {
    name: 'SII Misto',
    description: 'Fezes duras ou em caroços ≥25% e fezes moles ≥25% das evacuações'
...
export default function RomaIVCalculator({ onSaveResult, darkMode = true }: RomaIVCalculatorProps) {
  ...
  const checkRomaCriteria = () => {
    ...
    if (!answers['recurrent_pain']) {
      return { meets: false, reason: 'Critério obrigatório não atendido: dor abdominal recorrente' }
    }
    ...
    <div className="space-y-4">
      ...
      {romaResult.meets && (
        <div className="bg-green-900/20 border border-green-700 rounded-lg p-4">
          <h4 className="text-green-400 font-medium mb-3">Diagnóstico: Síndrome do Intestino Irritável</h4>
          
          <div className="space-y-3">
            <div>
              <p className="text-white font-medium mb-2">Selecione o subtipo baseado no padrão das fezes:</p>
                ...
                {Object.entries(siiSubtypes).map(([key, subtype]) => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      ...
                      {selectedSubtype === key && (
                        <Badge className="bg-green-600 text-white">Selecionado</Badge>
                      )}
                    </div>
                    <p className="text-gray-300 text-sm">{subtype.description}</p>
                  </div>
                ))}
              </div>
            </div>
      ...
      <div className="bg-blue-900/20 border border-blue-700 rounded-lg p-4">
        ...
        <div className="space-y-3 text-sm">
          {Object.entries(siiSubtypes).map(([key, subtype]) => (
            <div key={key}>
              <p className="font-medium text-white">{key} - {subtype.name}</p>
  ...
  return (
    ...
    >
      <div className="space-y-6">
        <div className="bg-blue-900/20 border border-blue-700 rounded-lg p-4">
          <h3 className="text-blue-400 font-medium mb-2">Critérios de Roma IV para SII</h3>
          <p className="text-gray-300 text-sm">
            Dor abdominal recorrente, em média, pelo menos 1 dia por semana nos últimos 3 meses, 
            associada com 2 ou mais dos seguintes critérios:
          </p>
        </div>
