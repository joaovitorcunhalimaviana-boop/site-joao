...
const pitPatterns = [
  {
    type: 'I',
    description: 'Normal ou arredondado - criptas regulares em tamanho e arranjo',
    histology: 'Mucosa normal',
    management: 'Nenhuma ação necessária'
  },
  {
    type: 'II',
    description: 'Estrelado - abertura das criptas em forma de estrela com arranjo uniforme',
    histology: 'Pólipos hiperplásicos (e lesões serrilhadas)',
    management: 'Polipectomia se >10mm'
  },
  {
    type: 'IIIL',
    description: 'Tubular grande - criptas com abertura tubular e alongada, arranjo regular',
    histology: 'Adenomas tubulares com baixo grau de displasia',
    management: 'Polipectomia ou EMR'
  },
  {
    type: 'IIIS',
    description: 'Tubular pequeno - criptas com menor diâmetro e arranjo compactado',
    histology: 'Adenomas tubulares com baixo grau de displasia',
    management: 'Polipectomia ou EMR'
  },
  {
  ...
  {
    type: 'Vi',
    description: 'Desestruturado irregular - padrão mais estruturado de criptas irregulares',
    histology: 'Adenomas com displasia de alto grau ou carcinoma com mínima invasão da submucosa',
    management: 'EMR ou ESD'
  },
  {
    type: 'Vn',
    description: 'Desestruturado não-estrutural - superfície rugosa com ulcerações e apagamento das criptas',
  ...
  return (
    ...
    >
      <div className="space-y-6">
        <CalculatorQuestion
          question="Padrão de Criptas (Pit Pattern)"
          required
        >
          <Select value={parameters.pitPattern} onValueChange={(value) => handleParameterChange('pitPattern', value)}>
            <SelectTrigger className="bg-gray-800 border-gray-600 text-white">
              <SelectValue placeholder="Selecione o padrão" />
            </SelectTrigger>
            <SelectContent className="bg-gray-800 border-gray-600">
              {pitPatterns.map((pattern) => (
                <SelectItem key={pattern.type} value={pattern.type} className="text-white hover:bg-gray-700">
                  <div className="flex flex-col">
                    <span className="font-medium">Tipo {pattern.type}</span>
                    <span className="text-sm text-gray-400">{pattern.description}</span>
                  </div>
                </SelectItem>
              ))}
            </SelectContent>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-blue-900/20 p-4 rounded-lg border border-blue-700">
                  <h4 className="font-semibold text-blue-300 mb-2">Padrão Identificado</h4>
                  <p className="text-white font-medium">Tipo {result.type}</p>
                  <p className="text-gray-300 text-sm">{result.description}</p>
                </div>
                
                <div className="bg-green-900/20 p-4 rounded-lg border border-green-700">
                  <h4 className="font-semibold text-green-300 mb-2">Histologia Esperada</h4>
        ...
        <CalculatorCard title="Interpretação dos Padrões">
          <div className="space-y-3">
            {pitPatterns.map((pattern) => (
              <div key={pattern.type} className="flex justify-between items-center py-2 border-b border-gray-700 last:border-b-0">
                <div>
                  <span className="font-medium text-white">Tipo {pattern.type}:</span>
                  <span className="text-gray-300 ml-2">{pattern.description}</span>
                </div>
                <span className="text-sm text-gray-400">{pattern.histology}</span>
              </div>
            ))}
          </div>
        </CalculatorCard>
