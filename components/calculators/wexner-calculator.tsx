...
export default function WexnerCalculator({
  ...
  const getScoreInterpretation = (score: number) => {
    if (score === 0)
      return { text: 'Continência perfeita', color: 'green' as const }
    if (score <= 9)
      return { text: 'Incontinência leve', color: 'yellow' as const }
    if (score <= 14)
      return { text: 'Incontinência moderada', color: 'yellow' as const }
    return { text: 'Incontinência severa', color: 'red' as const }
  }
