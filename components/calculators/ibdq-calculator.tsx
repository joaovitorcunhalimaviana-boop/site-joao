...
export default function IBDQCalculator({
  onSaveResult,
  darkMode = true,
}: IBDQCalculatorProps) {
  ...
  const totalPages = Math.ceil(ibdqQuestions.length / questionsPerPage)
  ...
  const getQualityOfLife = (score: number) => {
    if (score >= 170)
      return {
        level: 'Excelente',
  ...
  const qualityOfLife = getQualityOfLife(totalScore)
  const currentQuestions = ibdqQuestions.slice(
    currentPage * questionsPerPage,
    (currentPage + 1) * questionsPerPage
  )

  const bowelScore = calculateDomainScore('bowel')
  const systemicScore = calculateDomainScore('systemic')
  const emotionalScore = calculateDomainScore('emotional')
