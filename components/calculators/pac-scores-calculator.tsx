...
export default function PACScoresCalculator({
  onSaveResult,
  darkMode = true,
  ...
  const getSymInterpretation = (score: number) => {
  ...
  const getQolInterpretation = (score: number) => {
    if (score < 1)
      return {
  ...
  const isSymComplete =
    Object.keys(symAnswers).length === pacSymQuestions.length
  const isQolComplete =
    Object.keys(qolAnswers).length === pacQolQuestions.length
  const isComplete = isSymComplete && isQolComplete

  const symScore = calculatePACSymScore()
  const qolScore = calculatePACQolScore()
  const symInterpretation = getSymInterpretation(symScore)
