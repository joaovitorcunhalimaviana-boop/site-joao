...
const preparationScores = [
  {
    ...
    details: 'Mucosa do segmento não vista devido a fezes sólidas que não podem ser removidas'
  },
  {
    score: 1,
    description: 'Ruim',
    details: 'Porção da mucosa vista, mas outras áreas não são bem visualizadas devido a coloração, fezes residuais e/ou líquido opaco'
  },
  {
    score: 2,
    description: 'Bom',
    details: 'Coloração leve, pequenas quantidades de fezes residuais e/ou líquido opaco, mas mucosa bem vista'
  },
  {
    score: 3,
    description: 'Excelente',
    details: 'Toda mucosa bem vista, sem fezes residuais, líquido opaco ou coloração'
  }
]
...
const BostonCalculator: React.FC<{
  ...
  const getResult = () => {
    if (parameters.rightColon === -1 || parameters.transverseColon === -1 || parameters.leftColon === -1) {
      return null
    }
    
    const totalScore = parameters.rightColon + parameters.transverseColon + parameters.leftColon
    ...
    if (totalScore >= 6) {
      ...
      recommendation = 'Repetir colonoscopia em 1-3 anos ou considerar novo preparo'
    } else {
    ...
    const hasZeroScore = parameters.rightColon === 0 || parameters.transverseColon === 0 || parameters.leftColon === 0
    if (hasZeroScore) {
      quality = 'Preparo inadequado'
    ...
    >
      <div className="space-y-6">
        <CalculatorQuestion
          ...
          required
        >
          <Select value={parameters.rightColon.toString()} onValueChange={(value) => handleParameterChange('rightColon', value)}>
            <SelectTrigger className="bg-gray-800 border-gray-600 text-white">
              <SelectValue placeholder="Selecione a pontuação" />
            </SelectTrigger>
            <SelectContent className="bg-gray-800 border-gray-600">
              {preparationScores.map((score) => (
                <SelectItem key={score.score} value={score.score.toString()} className="text-white hover:bg-gray-700">
                  <div className="flex flex-col">
                    <span className="font-medium">{score.score} - {score.description}</span>
                    <span className="text-sm text-gray-400">{score.details}</span>
                  </div>
                </SelectItem>
              ))}
            </SelectContent>
        ...
        <CalculatorQuestion
          ...
          required
        >
          <Select value={parameters.transverseColon.toString()} onValueChange={(value) => handleParameterChange('transverseColon', value)}>
            <SelectTrigger className="bg-gray-800 border-gray-600 text-white">
              <SelectValue placeholder="Selecione a pontuação" />
            </SelectTrigger>
            <SelectContent className="bg-gray-800 border-gray-600">
        ...
        <CalculatorQuestion
          ...
          required
        >
          <Select value={parameters.leftColon.toString()} onValueChange={(value) => handleParameterChange('leftColon', value)}>
            <SelectTrigger className="bg-gray-800 border-gray-600 text-white">
              <SelectValue placeholder="Selecione a pontuação" />
            </SelectTrigger>
            <SelectContent className="bg-gray-800 border-gray-600">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-blue-900/20 p-4 rounded-lg border border-blue-700">
                  <h4 className="font-semibold text-blue-300 mb-2">Pontuação Total</h4>
                  <p className="text-2xl font-bold text-white">{result.totalScore}/9</p>
                ...
                <div className={`p-4 rounded-lg border ${
                  ...
                  <h4 className={`font-semibold mb-2 ${
                    result.totalScore >= 6 
                      ...
                      ? 'text-yellow-300'
                      : 'text-red-300'
                  }`}>Qualidade do Preparo</h4>
                  <p className="text-white font-medium">{result.quality}</p>
                  <p className="text-gray-300 text-sm mt-1">{result.adequacy}</p>
                </div>
                
                <div className="bg-orange-900/20 p-4 rounded-lg border border-orange-700">
                  <h4 className="font-semibold text-orange-300 mb-2">Recomendação</h4>
        ...
        <CalculatorCard title="Critérios de Pontuação">
          <div className="space-y-3">
            {preparationScores.map((score) => (
              <div key={score.score} className="flex items-start py-3 border-b border-gray-700 last:border-b-0">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center mr-3 text-white font-bold ${
                  score.score === 0 ? 'bg-red-600' :
                  score.score === 1 ? 'bg-orange-600' :
                  score.score === 2 ? 'bg-yellow-600' : 'bg-green-600'
                }`}>
                  {score.score}
                </div>
        ...
        <CalculatorCard title="Interpretação">
          <div className="text-gray-300 space-y-2">
            <p><strong className="text-green-400">6-9 pontos:</strong> Preparo adequado - Exame confiável</p>
            <p><strong className="text-yellow-400">4-5 pontos:</strong> Preparo limítrofe - Considerar repetição</p>
            <p><strong className="text-red-400">0-3 pontos:</strong> Preparo inadequado - Repetir exame</p>
            <p className="text-sm mt-3"><strong>Nota:</strong> Qualquer segmento com pontuação 0 torna o exame inadequado</p>
          </div>
        </CalculatorCard>
