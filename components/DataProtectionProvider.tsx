    ...
    try {
      ...
      */
      ...
      if (response && response.ok) {
        ...
        console.warn('⚠️ Sistema de proteção não disponível, aplicação funcionará normalmente')
        setIsProtectionActive(false)
        setProtectionStatus('inactive')
      }
      
    } catch (error) {
      console.warn('⚠️ SISTEMAS DE PROTEÇÃO NÃO DISPONÍVEIS, APLICAÇÃO FUNCIONARÁ NORMALMENTE:', error)
  ...
  const executeEmergencyBackup = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.warn(`⚠️ Backup falhou com status ${response.status}, mas sistema continuará funcionando`)
