...
function AgendaManagementPageContent() {
  ...
  const handleAddSlot = async () => {
    ...
    if (existingSlot) {
      showMessage('error', 'Já existe um horário agendado para esta data e hora.')
      return
    }
        ...
        >
          <div className="flex justify-between items-start mb-1">
            <span className={`text-sm font-medium ${
              isTodayDate ? 'text-blue-400' : isPast ? 'text-gray-500' : 'text-white'
            }`}>
              {day}
            </span>
            {daySlots.length > 0 && (
              ...
              <div className="flex items-center justify-between mb-6">
                <div>
                  ...
                  <p className="text-gray-300 mt-2">
                    Gerencie os horários disponíveis para agendamento por data específica
                  </p>
                </div>
                <button
                      ...
                      <div className="bg-blue-900/10 backdrop-blur-sm rounded-xl p-6 border border-blue-700/30">
                        ...
                        <div className="flex items-center justify-between mb-4">
                          <p className="text-gray-300">
                            Gerencie os horários disponíveis para esta data específica
                          </p>
                          <button
                            onClick={() => {
                              setNewSlot({ date: selectedDate, time: '09:00', displayDate: formatDateBR(selectedDate) })
                              ...
                              <div className="flex items-center">
                                <span
                                  className={`inline-block w-3 h-3 rounded-full mr-3 ${
                                    slot.isActive ? 'bg-green-400' : 'bg-gray-400'
                                  }`}
                                ></span>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    <div className="bg-green-900/20 backdrop-blur-sm rounded-xl p-6 border border-green-700/30 hover:bg-green-900/25 transition-all duration-300">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-green-300 text-sm font-medium">Horários Ativos</p>
                          <p className="text-2xl font-bold text-white">{activeSlots}</p>
                        </div>
                    ...
                    <div className="bg-gray-800/30 backdrop-blur-sm rounded-xl p-6 border border-gray-600/30 hover:bg-gray-800/40 transition-all duration-300">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-gray-300 text-sm font-medium">Horários Inativos</p>
                          <p className="text-2xl font-bold text-white">{inactiveSlots}</p>
                        </div>
                ...
                <input
                  ...
                  onChange={e => {
                    ...
                    setNewSlot({ 
                      ...
                      date: maskedValue.length === 10 ? parseDateBR(maskedValue) : ''
                    })
                  }}
