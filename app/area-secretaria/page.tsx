...
export default function AreaSecretaria() {
  ...
  const [activeTab, setActiveTab] = useState<'patients' | 'schedule'>(
    'patients'
  )
  ...
  const loadData = async () => {
    try {
      ...
      if (patientsResponse.ok) {
        ...
        console.error(
          'Erro na requisição de pacientes:',
  ...
  const handleCancelConsultation = async (consultationId: string) => {
    if (
      !confirm('Tem certeza que deseja cancelar e excluir este agendamento?')
    ) {
      return
    }
  ...
  const todayScheduledConsultations = todayConsultations.filter(
    consultation =>
      consultation.status === 'agendada' || consultation.status === 'confirmada'
  )

  // Consultas concluídas de hoje
  const todayCompletedConsultations = todayConsultations.filter(
    consultation => consultation.status === 'concluida'
  )

  // Consultas da data selecionada
  const selectedDateConsultations = (validConsultations || [])
    .filter(consultation => {
    ...
    <div className='min-h-screen bg-black'>
      ...
      <div className='relative isolate'>
                    ...
                    <div className='absolute right-0 mt-2 w-64 bg-gray-900/95 backdrop-blur-sm rounded-xl shadow-xl border border-gray-700 z-50'>
                      <div className='py-2'>
                        ...
                        <div className='px-4 py-2'>
                          ...
                          <button
                            onClick={() => {
                              router.push('/constipacao-intestinal')
                              setShowMenu(false)
                            }}
                            className='w-full text-left px-2 py-2 text-sm text-gray-300 hover:text-blue-400 hover:bg-gray-800/30 transition-colors rounded'
                          >
        ...
        <div className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6'>
          ...
          {/* Botão de Gestão de Agenda */}
          <div className='mb-8'>
            <button
              onClick={() => router.push('/area-medica/agenda?from=secretary')}
              className='w-full bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white px-6 py-4 rounded-2xl font-semibold text-lg shadow-lg border border-blue-500 transition-all duration-200 hover:shadow-xl flex items-center justify-center gap-3'
            >
              <svg
                className='w-6 h-6'
                fill='none'
                        ...
                        {selectedDateConsultations.length === 0 ? (
                          ...
                          selectedDateConsultations.map(consultation => (
                            ...
                            >
                              ...
                              <td className='px-6 py-4 whitespace-nowrap text-sm text-gray-300'>
                                {isoDateToBrazilianDisplay(consultation.date)}{' '}
                ...
                >
                  ...
                  <div>
                    ...
                    <BrazilianDateInput
                      ...
                      onChange={value =>
                        setNewConsultation({ ...newConsultation, date: value })
                      }
                      min={getTodayISO()}
                  ...
                  <div>
                    ...
                    <TimePicker
                      ...
                      onChange={time =>
                        setNewConsultation({ ...newConsultation, time })
                      }
                      className='w-full px-4 py-3 bg-gray-700/50 border border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-white transition-all'
                      placeholder='HH:MM'
                      required
