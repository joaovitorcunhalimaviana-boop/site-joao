...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      const whatsappResult = await sendWhatsAppNotification(type, patientData, appointmentData)
      notifications.push({
        channel: 'whatsapp',
        status: 'success',
        data: whatsappResult
    ...
    try {
      const telegramResult = await sendTelegramNotification(type, patientData, appointmentData)
      notifications.push({
        channel: 'telegram',
        status: 'success',
        data: telegramResult
    ...
    return NextResponse.json({
      success,
      message: success ? 'Notificações enviadas com sucesso' : 'Algumas notificações falharam',
      notifications,
...
function logNotificationDetails(
  ...
  console.log(`📅 Consulta: ${appointmentData.selectedDate || appointmentData.appointmentDate} às ${appointmentData.selectedTime || appointmentData.appointmentTime}`)
  console.log(`📱 WhatsApp: ${patientData.whatsapp}`)
  
  console.log('\n📤 CANAIS DE NOTIFICAÇÃO:')
  notifications.forEach((notification, index) => {
    const status = notification.status === 'success' ? '✅' : '❌'
    console.log(`${index + 1}️⃣ ${status} ${notification.channel.toUpperCase()}: ${notification.status}`)
