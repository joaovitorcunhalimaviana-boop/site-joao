...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!recipients || recipients.length === 0) {
      try {
        ...
        emailList = patientsData
          .filter((patient: any) => patient.email && patient.email.trim() !== '')
          .map((patient: any) => patient.email);
          
        console.log(`Encontrados ${emailList.length} pacientes com email para newsletter`);
      } catch (error) {
    ...
    if (emailSent) {
      console.log(`✅ Newsletter enviada com sucesso para ${emailList.length} destinatários`);
      ...
      try {
        ...
        if (fs.existsSync(newsletterPath)) {
          newsletterHistory = JSON.parse(fs.readFileSync(newsletterPath, 'utf8'));
        }
        ...
        fs.writeFileSync(newsletterPath, JSON.stringify(newsletterHistory, null, 2));
...
export async function GET() {
  try {
    ...
    const newsletterHistory = JSON.parse(fs.readFileSync(newsletterPath, 'utf8'));
    
    return NextResponse.json({
      success: true,
      newsletters: newsletterHistory.sort((a: any, b: any) => 
