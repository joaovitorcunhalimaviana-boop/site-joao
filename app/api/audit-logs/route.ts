...
class AuditLogSystem {
  ...
  static async logEvent(
    ...
    try {
      ...
      if (logEntry.severity === 'CRITICAL') {
        console.log('🚨 EVENTO CRÍTICO REGISTRADO:', JSON.stringify(logEntry, null, 2))
      }
      
    } catch (error) {
      console.error('❌ ERRO AO REGISTRAR LOG DE AUDITORIA:', error)
      // Em caso de erro, pelo menos registrar no console
      console.log('📝 EVENTO (FALLBACK):', { action, resource, details, options })
    }
  }
  ...
  static async searchLogs(query: AuditQuery): Promise<{ logs: AuditLog[], total: number }> {
    try {
      const logsDir = path.join(process.cwd(), 'logs', 'audit')
      ...
      const files = fs.readdirSync(logsDir)
      const logFiles = files.filter(file => file.startsWith('audit_') && file.endsWith('.json'))
      ...
      if (query.startDate || query.endDate) {
        filesToRead = logFiles.filter(file => {
          ...
          if (query.startDate && fileDate < new Date(query.startDate)) return false
  ...
  static async logPatientEvent(action: string, patientId: string, details: any, options: any = {}) {
  ...
  static async logAppointmentEvent(action: string, appointmentId: string, details: any, options: any = {}) {
  ...
  static async logMedicalRecordEvent(action: string, recordId: string, details: any, options: any = {}) {
  ...
  static async logSecurityEvent(action: string, details: any, options: any = {}) {
