...
export async function GET(request: NextRequest) {
  try {
    ...
    const integrityCheck = await enhancedBackupSystem.performDataIntegrityCheck()
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        integrity: {
          ...
          criticalIssues: integrityCheck.issues.filter(i => i.severity === 'CRITICAL').length
        },
        services: {
  ...
  try {
    ...
    switch (action) {
      ...
      case 'integrity_check':
        const integrityResult = await enhancedBackupSystem.performDataIntegrityCheck()
