...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      for (const patientData of patients) {
        try {
          ...
          console.error(`❌ Erro ao migrar paciente ${patientData.name}:`, error)
    ...
    try {
      const appointmentsFile = path.join(process.cwd(), 'data', 'unified-appointments.json')
      ...
      for (const appointmentData of appointments) {
        try {
          ...
          if (!patient) {
            console.log(`⚠️ Paciente não encontrado para agendamento: ${appointmentData.patientId}`)
          ...
          await prisma.appointment.create({
            data: {
              ...
              date: new Date(`${appointmentData.date}T${appointmentData.time}:00`),
              time: appointmentData.time,
              type: type as any,
              status: status as any,
              reason: appointmentData.notes,
              createdAt: appointmentData.createdAt ? new Date(appointmentData.createdAt) : new Date(),
              updatedAt: appointmentData.updatedAt ? new Date(appointmentData.updatedAt) : new Date()
            }
          })
          ...
          console.error(`❌ Erro ao migrar agendamento ${appointmentData.id}:`, error)
          results.appointments.errors++
        }
      }
    } catch (error) {
    ...
    // Calcular totais
    results.total.migrated = results.patients.migrated + results.appointments.migrated
