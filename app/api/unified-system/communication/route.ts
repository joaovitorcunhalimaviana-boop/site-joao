...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    let contacts = getAllCommunicationContacts()
    ...
    if (subscribed !== null) {
      ...
      contacts = contacts.filter(contact => 
        contact.emailPreferences.subscribed === isSubscribed
      )
    }
    ...
    const stats = {
      ...
      newsletterSubscribers: contacts.filter(c => c.emailPreferences.newsletter).length,
      sources: {
        newsletter: contacts.filter(c => c.registrationSources.includes('newsletter')).length,
        public_appointment: contacts.filter(c => c.registrationSources.includes('public_appointment')).length,
        doctor_area: contacts.filter(c => c.registrationSources.includes('doctor_area')).length,
        secretary_area: contacts.filter(c => c.registrationSources.includes('secretary_area')).length
      }
    }
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (!body.source || !['newsletter', 'public_appointment', 'doctor_area', 'secretary_area'].includes(body.source)) {
      return NextResponse.json({
        success: false,
    ...
    const result = createOrUpdateCommunicationContact({
      ...
      source: body.source,
      emailPreferences: {
        newsletter: body.emailPreferences?.newsletter ?? (body.source === 'newsletter'),
...
export async function PUT(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (body.emailPreferences) {
      ...
      if (body.emailPreferences.subscribed === false && contact.emailPreferences.subscribed === true) {
        updateData.emailPreferences.unsubscribedAt = new Date().toISOString()
      }
      
      // Se está se inscrevendo novamente, marcar data
      if (body.emailPreferences.subscribed === true && contact.emailPreferences.subscribed === false) {
    ...
    if (body.whatsappPreferences) {
      ...
      if (body.whatsappPreferences.subscribed === true && contact.whatsappPreferences.subscribed === false) {
