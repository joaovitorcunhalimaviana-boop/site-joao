...
export function useCalculator(config: CalculatorConfig) {
  ...
  const validateField = useCallback((field: CalculatorField, value: any): string | null => {
  ...
  const isComplete = useMemo(() => {
    ...
    return validationErrors.length === 0 && completedFields.length === requiredFields.length
  ...
  const saveResult = useCallback(() => {
    if (result && isComplete) {
      ...
      try {
        ...
        localStorage.setItem(`calculator_${config.name}_results`, JSON.stringify(updated))
  ...
  const exportResult = useCallback((format: 'pdf' | 'csv' | 'json', resultToExport?: CalculatorResult) => {
  ...
  const convertToCSV = (data: CalculatorResult): string => {
    ...
    const rows = [
      ...
      ['Tempo de Cálculo (ms)', data.metrics?.calculationTime?.toFixed(2) || 'N/A'],
      ['Mudanças de Campo', data.metrics?.fieldChanges?.toString() || 'N/A'],
      ['Taxa de Completude (%)', data.metrics?.completionRate?.toFixed(1) || 'N/A']
    ]
...
export const calculatorUtils = {
  ...
  interpretScore: (score: number, ranges: { min: number; max: number; label: string; description: string }[]) => {
