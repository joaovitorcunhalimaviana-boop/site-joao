...
export const PatientSchema = z.object({
  ...
  whatsapp: z.string().regex(/^\(?\d{2}\)?\s?\d{4,5}-?\d{4}$/, 'Formato de WhatsApp inválido'),
  ...
  insurance: z.object({
    type: z.enum(['unimed', 'particular', 'outro']).refine(
      (val) => ['unimed', 'particular', 'outro'].includes(val),
...
export const AppointmentSchema = z.object({
  ...
  patientPhone: z.string().regex(/^\(?\d{2}\)?\s?\d{4,5}-?\d{4}$/, 'Telefone inválido'),
  patientWhatsapp: z.string().regex(/^\(?\d{2}\)?\s?\d{4,5}-?\d{4}$/, 'WhatsApp inválido'),
  patientEmail: z.string().email('Email inválido').optional(),
  patientBirthDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Data de nascimento inválida').optional(),
  insuranceType: z.enum(['unimed', 'particular', 'outro']).refine(
    (val) => ['unimed', 'particular', 'outro'].includes(val),
    { message: 'Tipo de convênio inválido' }
  ),
  insurancePlan: z.string().optional(),
  appointmentDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'Data do agendamento inválida'),
  appointmentTime: z.string().regex(/^\d{2}:\d{2}$/, 'Horário inválido (formato HH:MM)'),
  appointmentType: z.enum(['consulta', 'retorno', 'urgencia', 'teleconsulta', 'visita_domiciliar']).refine(
    (val) => ['consulta', 'retorno', 'urgencia', 'teleconsulta', 'visita_domiciliar'].includes(val),
    { message: 'Tipo de consulta inválido' }
  ),
  status: z.enum(['agendada', 'confirmada', 'em_andamento', 'concluida', 'cancelada', 'reagendada']).refine(
    (val) => ['agendada', 'confirmada', 'em_andamento', 'concluida', 'cancelada', 'reagendada'].includes(val),
    { message: 'Status inválido' }
  ),
  source: z.enum(['public_form', 'doctor_area', 'secretary_area']).refine(
...
export const UserSchema = z.object({
  ...
  role: z.enum(['doctor', 'secretary', 'admin']).refine(
    (val) => ['doctor', 'secretary', 'admin'].includes(val),
...
export const MedicalCalculatorSchema = z.object({
  calculatorType: z.enum(['cdai', 'mayo', 'wexner', 'bristol', 'pac-scores', 'ibdq', 'constipacao', 'st-marks']).refine(
    (val) => ['cdai', 'mayo', 'wexner', 'bristol', 'pac-scores', 'ibdq', 'constipacao', 'st-marks'].includes(val),
...
export const NotificationSchema = z.object({
  type: z.enum(['email', 'whatsapp', 'telegram']).refine(
    (val) => ['email', 'whatsapp', 'telegram'].includes(val),
...
export function validateData<T>(schema: z.ZodSchema<T>, data: unknown): { success: true; data: T } | { success: false; errors: string[] } {
  try {
    ...
    if (error instanceof z.ZodError) {
      try {
        const errors = error.issues.map(err => `${err.path.join('.')}: ${err.message}`) || ['Erro de validação'];
