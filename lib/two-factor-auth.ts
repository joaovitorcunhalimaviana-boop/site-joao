...
export class TwoFactorAuthService {
   ...
   */
  static async setupTwoFactor(data: z.infer<typeof SetupTwoFactorSchema>, clientIp?: string): Promise<TwoFactorSetupResult> {
    try {
      ...
      if (user.twoFactorEnabled) {
        return { success: false, error: '2FA já está habilitado para este usuário' }
      }
      ...
      const backupCodes = validatedData.backupCodes || this.generateBackupCodes()
  ...
  static async verifyAndActivateTwoFactor(
    ...
    try {
      ...
      if (setup.expiresAt < new Date()) {
        ...
        return { success: false, error: 'Configuração 2FA expirou. Inicie novamente.' }
      }
      ...
      if (validatedData.isBackupCode) {
        ...
        isValid = await this.verifyBackupCode(validatedData.token, JSON.parse(setup.backupCodes))
      } else {
  ...
  static async verifyTwoFactor(
    ...
    try {
      ...
      await AuditService.log({
        userId: validatedData.userId,
        action: isValid ? 'TWO_FACTOR_VERIFIED' : 'TWO_FACTOR_VERIFICATION_FAILED',
        resource: 'TwoFactorAuth',
        details: JSON.stringify({ 
          email: user.email,
          clientIp,
   ...
   */
  static async getTwoFactorStatus(userId: string): Promise<TwoFactorStatusResult> {
    try {
      const user = await prisma.user.findUnique({
        where: { id: userId },
        select: {
      ...
      return {
        ...
        backupCodesCount: user.twoFactorBackupCodes ? JSON.parse(user.twoFactorBackupCodes).length : 0,
  ...
  private static async verifyBackupCode(code: string, hashedCodes: string[]): Promise<boolean> {
    const bcrypt = require('bcryptjs')
