...
export class DuplicateDetectionService {
  ...
  static async detectDuplicateByCPF(cpf: string): Promise<DuplicateDetectionResult> {
    ...
    try {
      // Buscar todos os pacientes para verificar CPF criptografado
  ...
  static async detectDuplicateBySimilarity(
    ...
    try {
      ...
      for (const patient of allPatients) {
        ...
        try {
          const decryptedPhone = LGPDEncryptionService.decrypt(patient.phone)
          const patientNormalizedPhone = decryptedPhone.replace(/[^\d]/g, '')
          if (normalizedPhone === patientNormalizedPhone) {
            similarity += 0.3
            matchReasons.push(`Telefone idêntico: ${decryptedPhone}`)
          }
        } catch (error) {
        ...
        if (email && patient.email) {
          try {
            const decryptedEmail = LGPDEncryptionService.decrypt(patient.email)
            if (normalizedEmail === decryptedEmail.toLowerCase().trim()) {
              similarity += 0.1
              matchReasons.push(`Email idêntico: ${decryptedEmail}`)
            }
          } catch (error) {
            // Ignorar erro de descriptografia
          }
