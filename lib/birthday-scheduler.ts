...
export async function checkAndProcessBirthdays(): Promise<BirthdayCheckResult> {
  ...
  try {
    ...
    const patientsWithEmailAndBirthday = patientsData.filter((patient: any) => 
    ...
    const todayBirthdays = patientsWithEmailAndBirthday.filter((patient: any) => {
...
export function initializeBirthdayScheduler(): void {
  ...
  console.log(`⏰ Próxima verificação agendada para: ${scheduledTime.toLocaleString('pt-BR')}`);
}

// Função para verificar aniversários manualmente (para testes)
export async function manualBirthdayCheck(): Promise<{ count: number; patients: any[] }> {
  try {
