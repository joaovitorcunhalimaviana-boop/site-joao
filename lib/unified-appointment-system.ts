...
import { sendTelegramAppointmentNotification, type AppointmentNotificationData } from './telegram-notifications'
...
async function loadFromStorage<T>(key: string): Promise<T[]> {
  ...
  if (typeof window === 'undefined') {
    ...
    try {
      ...
      console.log(`✅ Dados carregados do arquivo [${key}]: ${Array.isArray(data) ? data.length : 'objeto'} itens`)
  ...
  try {
    ...
    if (!item) {
      console.log(`📭 Nenhum dado encontrado no localStorage [${key}], tentando recuperar do backup`)
      
      // Se for a chave de pacientes e não houver dados, tentar recuperar do backup
      if (key === PATIENTS_KEY) {
        const patients = await loadPatientBackup()
         if (patients.length > 0) {
           console.log(`🔄 Backup de pacientes carregado: ${patients.length} pacientes`)
    ...
    console.log(`✅ Dados carregados do localStorage [${key}]:`, Array.isArray(parsed) ? `${parsed.length} itens` : 'objeto')
...
function getFromStorage<T>(key: string, defaultValue: T): T {
  ...
  try {
    if (typeof window === 'undefined' || !window.localStorage) {
      console.log(`📱 localStorage não disponível (servidor) - retornando valor padrão para [${key}]`)
      return defaultValue
    }
    ...
    if (!item) {
      ...
      // Se for a chave de pacientes e não houver dados, tentar recuperar do backup
      if (key === PATIENTS_KEY) {
        loadPatientBackup().then(async (patients) => {
          if (patients.length > 0) {
            console.log(`🔄 Recuperando ${patients.length} pacientes do backup`)
  ...
  try {
    ...
    if (typeof window === 'undefined') {
      ...
      try {
        ...
        console.log('✅ Backup de pacientes carregado do arquivo:', patients.length, 'pacientes')
        return patients
      } catch (fileError) {
      ...
      if (response.ok) {
        ...
        console.log('✅ Backup de pacientes carregado via API:', data.count, 'pacientes')
...
async function saveToStorage<T>(key: string, data: T): Promise<void> {
  ...
  if (typeof window === 'undefined') {
    ...
    if (Array.isArray(data)) {
      try {
        ...
        console.log(`✅ Dados salvos no arquivo [${key}]: ${(data as any[]).length} itens`)
      } catch (error) {
  ...
  try {
    console.log(`🔄 Iniciando salvamento no localStorage [${key}]:`, Array.isArray(data) ? `${(data as any[]).length} itens` : 'objeto')
    ...
    if (typeof window !== 'undefined' && window.localStorage) {
      ...
      if (!saved) {
        console.error(`❌ Falha ao verificar dados salvos no localStorage [${key}]`)
      } else {
        const parsedSaved = JSON.parse(saved)
        console.log(`✅ Verificação confirmada: ${Array.isArray(parsedSaved) ? parsedSaved.length : 'objeto'} no localStorage [${key}]`)
        
        // Para pacientes, verificar se o último adicionado está lá
        if (key === PATIENTS_KEY && Array.isArray(data) && Array.isArray(parsedSaved)) {
          const originalCount = (data as any[]).length
          const savedCount = parsedSaved.length
          console.log(`📊 Contagem: Original=${originalCount}, Salvo=${savedCount}`)
          
          if (originalCount > 0) {
            const lastOriginal = (data as any[])[originalCount - 1]
            const lastSaved = parsedSaved.find((p: any) => p.id === lastOriginal.id)
            if (lastSaved) {
              console.log(`✅ Último paciente confirmado no localStorage: ${lastSaved.id}`)
            } else {
              console.error(`❌ Último paciente NÃO encontrado no localStorage: ${lastOriginal.id}`)
            }
          }
        }
      }
...
export async function createAppointment(appointmentData: Omit<UnifiedAppointment, 'id' | 'createdAt' | 'updatedAt'>): Promise<{ success: boolean; appointment?: UnifiedAppointment; error?: string }> {
  try {
    ...
    if (conflictingAppointment) {
      return {
        success: false,
        error: 'Já existe um agendamento para este paciente nesta data e horário'
      }
    }
    ...
    console.log(`✅ Agendamento criado: ${newAppointment.patientName} - ${newAppointment.appointmentDate} ${newAppointment.appointmentTime}`)
    ...
    // Enviar notificação via Telegram
     try {
       const notificationData: AppointmentNotificationData = {
         ...
         insuranceType: newAppointment.insuranceType as 'unimed' | 'particular' | 'outro',
...
export async function getDailyAgenda(date: string): Promise<DailyAgenda> {
  ...
  return {
    ...
    confirmedAppointments: appointments.filter(apt => apt.status === 'confirmada').length,
    pendingAppointments: appointments.filter(apt => apt.status === 'agendada').length,
    completedAppointments: appointments.filter(apt => apt.status === 'concluida').length
  }
}

// Atualizar status do agendamento
export async function updateAppointmentStatus(appointmentId: string, status: UnifiedAppointment['status'], notes?: string): Promise<{ success: boolean; error?: string }> {
  try {
    const appointments = await getAllAppointments()
...
export async function updateAppointment(appointmentId: string, updateData: {
  date: string;
  time: string;
  type: 'consulta' | 'retorno' | 'urgencia' | 'teleconsulta' | 'visita_domiciliar';
  notes?: string;
...
export async function getNextMedicalRecordNumber(): Promise<number> {
  ...
  const maxRecordNumber = Math.max(...patients.map(p => p.medicalRecordNumber || 0))
  return maxRecordNumber + 1
}

// Criar ou atualizar paciente
export async function createOrUpdatePatient(patientData: Omit<Patient, 'id' | 'createdAt' | 'updatedAt'>): Promise<{ success: boolean; patient?: Patient; error?: string }> {
  try {
    ...
    if (!validateCPF(patientData.cpf)) {
      ...
      return { success: false, error: 'CPF inválido. Verifique o número digitado.' }
    }
    ...
    if (existingPatientByCpf) {
      console.log('🔄 Paciente encontrado pelo CPF, atualizando dados:', existingPatientByCpf.id)
      ...
      const patientIndex = patients.findIndex(p => p.id === existingPatientByCpf.id)
      patients[patientIndex] = updatedPatient
      
      await saveToStorage(PATIENTS_KEY, patients)
      console.log('✅ Paciente atualizado com sucesso pelo CPF:', updatedPatient.id, 'Prontuário:', updatedPatient.medicalRecordNumber)
      ...
      await saveToStorage(PATIENTS_KEY, patients)
      console.log('✅ Novo paciente criado com sucesso:', newPatient.id, 'Prontuário:', medicalRecordNumber)
      ...
      if (newPatient.email) {
        try {
          console.log('📧 Enviando e-mail de boas-vindas para:', newPatient.email)
          ...
          if (typeof window === 'undefined') {
            ...
            if (emailSent) {
              console.log('✅ E-mail de boas-vindas enviado com sucesso (servidor)')
            } else {
              console.warn('⚠️ Falha ao enviar e-mail de boas-vindas (servidor)')
            }
          } else {
            ...
            if (emailResponse.ok) {
              ...
              if (emailResult.success) {
                console.log('✅ E-mail de boas-vindas enviado com sucesso (cliente)')
              } else {
                console.warn('⚠️ Falha ao enviar e-mail de boas-vindas:', emailResult.message)
              }
            } else {
      ...
      setTimeout(() => {
        if (typeof window !== 'undefined' && window.localStorage) {
          ...
          if (localStorageData) {
            ...
            const localStoragePatient = parsedData.find((p: Patient) => p.id === newPatient.id)
            if (localStoragePatient) {
              console.log('✅ Paciente verificado no localStorage:', localStoragePatient.name)
            } else {
...
export async function getAllPatients(): Promise<Patient[]> {
  try {
    ...
    for (const apt of appointments) {
      ...
      if (!existingPatientIds.has(apt.patientId) && !processedPatientIds.has(apt.patientId)) {
    ...
    console.log(`📋 Total de pacientes encontrados: ${allPatients.length} (${patients.length} do arquivo + ${appointmentPatients.length} dos agendamentos)`)
...
export async function createPublicAppointment(formData: {
  ...
  selectedDate: Date
  selectedTime: string
}): Promise<{ success: boolean; appointment?: UnifiedAppointment; patient?: Patient; error?: string; existingAppointment?: UnifiedAppointment }> {
  try {
    ...
    const existingAppointment = existingAppointments.find(apt => 
      ...
      (
        ...
        (apt.patientEmail && formData.email && apt.patientEmail.toLowerCase() === formData.email.toLowerCase())
      )
    )
    ...
    console.log('👤 Resultado do paciente:', patientResult.success ? 'SUCESSO' : 'FALHA', patientResult.error)
    ...
    console.log('📅 Resultado do agendamento:', appointmentResult.success ? 'SUCESSO' : 'FALHA', appointmentResult.error)
...
export async function cancelAppointment(appointmentId: string): Promise<{ success: boolean; error?: string }> {
  try {
    ...
    console.log(`✅ Agendamento cancelado: ${appointments[appointmentIndex].patientName}`)
...
export async function getSystemStats(): Promise<{
  ...
  // Usar fuso horário do Brasil para obter a data correta
  const today = new Date().toLocaleDateString('pt-BR', {
    timeZone: 'America/Sao_Paulo',
    year: 'numeric',
    month: '2-digit',
    day: '2-digit'
  }).split('/').reverse().join('-') // Converter de DD/MM/YYYY para YYYY-MM-DD
  
  console.log('Data atual (Brasil):', today)
  console.log('Agendamentos encontrados:', appointments.map(apt => ({ date: apt.appointmentDate, patient: apt.patientName })))
  ...
  return {
    ...
    pendingAppointments: todayAppointments.filter(apt => apt.status === 'agendada').length,
    completedAppointments: todayAppointments.filter(apt => apt.status === 'concluida').length
  }
}
