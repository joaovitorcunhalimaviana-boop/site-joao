...
export function validateField(value: any, rules: ValidationRule): ValidationResult {
  ...
  if (rules.min !== undefined && Number(value) < rules.min) {
    errors.push(rules.message || defaultMessages.min.replace('{min}', String(rules.min)))
  }
  
  // Validar valor máximo
  if (rules.max !== undefined && Number(value) > rules.max) {
    errors.push(rules.message || defaultMessages.max.replace('{max}', String(rules.max)))
  }
  
  // Validar comprimento mínimo
  if (rules.minLength !== undefined && String(value).length < rules.minLength) {
    errors.push(rules.message || defaultMessages.minLength.replace('{minLength}', String(rules.minLength)))
  }
  
  // Validar comprimento máximo
  if (rules.maxLength !== undefined && String(value).length > rules.maxLength) {
    errors.push(rules.message || defaultMessages.maxLength.replace('{maxLength}', String(rules.maxLength)))
  }
...
export function validateForm(data: Record<string, any>, validationRules: FieldValidation): Record<string, ValidationResult> {
...
export function useValidation(initialRules: FieldValidation = {}) {
  ...
  const validateAllFields = React.useCallback((data: Record<string, any>) => {
